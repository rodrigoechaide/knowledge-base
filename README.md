# Knowledge Base

## Code Editors

### Vim

- [vim cheat sheet](code-editors/vim/cheat-sheet.md)

### Vscode

- [python cheat sheet](code-editors/vscode/python/cheat-sheet.md)

## Docker

- [docker cheat sheet](docker/cheat-sheet.md)

## Flux

- [flux cheat sheet](flux/cheat-sheet.md)

## Helm

- [helm cheat sheet](helm/cheat-sheet.md)

## Kubernetes

- [kubernetes cheat sheet](kubernetes/cheat-sheet.md)

## Maven

- [maven cheat sheet](maven/cheat-sheet.md)

## Software Configuration Management

- [git cheat sheet](scm/git/cheat-sheet.md)
- [semantic commit messages](scm/git/semantic-commit-messages.md)

## Tech Radar

- [tech radar](tech-radar/tech-radar.md)

## Unix CLI Tools and Commands

### Networking

- [linux networking tools](unix-cli-useful-tools-commands/networking/linux-networking-tools.md)
- [curl cheat sheet](unix-cli-useful-tools-commands/networking/curl.md)
- [wget cheat sheet](unix-cli-useful-tools-commands/networking/wget.md)

### Package Managers

- [yum cheat sheet](unix-cli-useful-tools-commands/package-managers/yum.md)

### Performance Testing

- [apache benchmark cheat sheet](unix-cli-useful-tools-commands/performance-testing/apache-benchmark.md)
- [apache jmeter cheat sheet](unix-cli-useful-tools-commands/performance-testing/apache-jmeter.md)
- [stress-ng cheat sheet](unix-cli-useful-tools-commands/performance-testing/stress-ng.md)
- [stress cheat sheet](unix-cli-useful-tools-commands/performance-testing/stress.md)

### PKI

- [cfssl cheat sheet](unix-cli-useful-tools-commands/pki/cfssl.md)
- [openssl cheat sheet](unix-cli-useful-tools-commands/pki/openssl.md)
- [step cheat sheet](unix-cli-useful-tools-commands/pki/step.md)

## Workspace Setup

- [linux](workspace-setup/linux.md)
- [macos](workspace-setup/macos.md)
- [windows](workspace-setup/windows.md)
